...
// Telegram Mini Apps Authentication Middleware
function authenticateTMA(req, res, next) {
    ...
    switch (authType) {
        case 'tma':
            try {
                ...
                db.get(userQuery, [initData.user.id], (err, user) => {
                    if (err) {
                ...
                return res.status(401).json({ error: 'Invalid init data: ' + e.message });
            }
            break;
        default:
...
function convertKeyboardLayout(text) {
    ...
    if (result === text) {
        for (const [en, ru] of Object.entries(enToRu)) {
            result = result.replace(new RegExp(en.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), ru);
        }
    }
...
function reverseTransliterate(text) {
    ...
    for (const key of sortedKeys) {
        result = result.replace(new RegExp(key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), reverseTranslitMap[key]);
    }
